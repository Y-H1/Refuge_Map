<?php
    ...
    class Hoge {
        ...
        /*public function lat_lng() {
            for($i = 1; $i <= 5; $i++) {
                $json_url = "https://maps.googleapis.com/maps/api/geocode/json?address=" . $_SESSION["address".$i] . "&key=AIzaSyBRwF0eEILxYC9Im1Cx9H-EWbfZc2GRluc";
                $rawdata = file_get_contents($json_url);
                $json_array = json_decode($rawdata,TRUE);
                //var_dump($json_array["results"][0]["geometry"]["location"]["lat"]);
                //var_dump($json_array["results"][0]["geometry"]["location"]["lng"]);

                $_SESSION["lat".$i] = $json_array["results"][0]["geometry"]["location"]["lat"];
                $_SESSION["lng".$i] = $json_array["results"][0]["geometry"]["location"]["lng"];
            }
        }*/
        ...
        public function result($x,$s) {
            ...
            global $lat_array,$lng_array,$name_array,$mode_count;
            ...
            while($x[count($x) - 1] == "") {
                ...
                $query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from aed order by distance asc limit '".$count."',1";
                    if(!empty($_POST["radio"]) && !empty($_POST["nami"])) {
                        $query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from aed where type = '公共施設' and eigyo_time = '24時間365日' order by distance asc limit '".$count."',1";
                    }else if(!empty($_POST["radio"])) {
                        $query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from aed where type = '公共施設' order by distance asc limit '".$count."',1";
                    }else if(!empty($_POST["nami"])) {
                        $query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from aed where eigyo_time = '24時間365日' order by distance asc limit '".$count."',1";
                    }
        ...
        public function select(){
            ...
            if (!$db) {
                ...
                $serch = $_POST['name'];
                $db->quote($serch);
                $query = "SELECT count(*) as count FROM aed where name = '" . $serch . "'";
                $result = $db->query($query);
                if (!$result) {
                    ...
                    if($check == 0) {
                        ...
                        $query = "SELECT * FROM aed where name = '" . $serch . "'";
                        $result = $db->query($query);
                        if (!$result) {
                            print('クエリーが失敗しました。' . $db->error);
                            $db = null;
    ...
    $query = "SELECT count(*) as count FROM aed";
    $result = $db->query($query);
    if (!$result) {
        print('クエリーが失敗しました。');
        exit();
        ...
        for($i = 0; $i < count($array); $i++) {
            ...
            $query = "SELECT * FROM aed limit ".$i.",1";
            $result = $db->query($query);
