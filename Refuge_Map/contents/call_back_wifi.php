<?php
	class Hoge {
		...
		/*public function check($column) {
			$dsn ='sqlite:C:/xampp/htdocs/避難マップくん/sample.db';
			$db = new PDO($dsn, '', '');
		...
		public function result($x,$s) {
			...
			while($x[count($x) - 1] == "") {
				...
				$query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from wifi order by distance asc limit '".$count."',1";
					if(!empty($_POST["radio"]) && !empty($_POST["nami"])) {
						$query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from wifi where type = '公共施設' and eigyo_time = '24時間365日' order by distance asc limit '".$count."',1";
					}else if(!empty($_POST["radio"])) {
						$query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from wifi where type = '公共施設' order by distance asc limit '".$count."',1";
					}else if(!empty($_POST["nami"])) {
						$query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from wifi where eigyo_time = '24時間365日' order by distance asc limit '".$count."',1";
					}
		...
		public function select(){
			...
			if (!$db) {
				...
				$serch = $_POST['name'];
				$db->quote($serch);
				$query = "SELECT count(*) as count FROM wifi where name = '" . $serch . "'";
				$result = $db->query($query);
				if (!$result) {
					...
					if($check == 0) {
						...
						$query = "SELECT * FROM wifi where name = '" . $serch . "'";
						$result = $db->query($query);
						if (!$result) {
							print('クエリーが失敗しました。' . $db->error);
							$db = null;
	...
	if (!$result) {
		...
		for($i = 0; $i < count($array); $i++) {
			...
			$query = "SELECT * FROM wifi limit ".$i.",1";
			$result = $db->query($query);
		...
		<div class="wrap">
			...
			<form id="Form" name="Form" action="" method="POST">
				<div class="d_h1">
					...
					<p><input type="text" id="automplete2" name="name" placeholder="   検索" value="<?php echo $serch;?>">
					<input type="submit" id="search" name="sousin" value="検索">
					<input type="submit" name="route" value="ルート案内" onclick="kakunin(0);"><?php echo "<font color='red'>".$errormessage."</font>";?></p>
				</div>
