<?php
	...
	class Hoge {
		...
		public function result($x,$s) {
			...
			while($x[count($x) - 1] == "") {
				...
				if($count == 0) {
					$query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from water order by distance asc limit '".$count."',1";
				}else {
					$query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from water ".$not_in." order by distance asc limit '".$count."',1";
				}
				//echo $query;
				$sql = $db->query($query);
		...
		public function select(){
			...
			if (!$db) {
				...
				$serch = $_POST['name'];
				$db->quote($serch);
				$query = "SELECT count(*) as count FROM water where name = '" . $serch . "'";
				...
				if (!$result) {
					...
					if($check == 0) {
						...
						$query = "SELECT * FROM water where name = '" . $serch . "'";
						$result = $db->query($query);
						if (!$result) {
							print('クエリーが失敗しました。' . $db->error);
							$db = null;
	...
	if (!$result) {
		...
		for($i = 0; $i < count($array); $i++) {
			...
			$query = "SELECT * FROM water limit ".$i.",1";
			$result = $db->query($query);
