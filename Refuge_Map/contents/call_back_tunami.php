<?php
	...
	class Hoge {
		...
		public function result($x,$s) {
			...
			while($x[count($x) - 1] == "") {
				...
				$query = "select * ,((lat - '".$latitudecd."') * (lat - '".$latitudecd."') + (lng - '".$longitudecd."') * (lng - '".$longitudecd."')) as distance from tunami order by distance asc limit '".$count."',1";
		...
		public function select(){
			...
			if (!$db) {
				...
				$serch = $_POST['name'];
				$db->quote($serch);
				$query = "SELECT count(*) as count FROM tunami where name = '" . $serch . "'";
				...
				if (!$result) {
					...
					if($check == 0) {
						...
						$query = "SELECT * FROM tunami where name = '" . $serch . "'";
						$result = $db->query($query);
						if (!$result) {
							print('クエリーが失敗しました。' . $db->error);
							$db = null;
	...
	$query = "SELECT DISTINCT count(*) as count FROM tunami";
	$result = $db->query($query);
	if (!$result) {
		print('クエリーが失敗しました。');
		exit();
		...
		for($i = 0; $i < count($array); $i++) {
			...
			$query = "SELECT DISTINCT * FROM tunami limit ".$i.",1";
			$result = $db->query($query);
